...
def create_configuration(client_id: Optional[str] = None, client_secret: Optional[str] = None) -> SHConfig:
    config = SHConfig()
...
def generate_name(ids: list[str]) -> str:
    ...
    extract_element = lambda index : "".join(list({substring[index] for substring in substrings}))
    ...
    return "_".join([satellite_name, preprocessing_level, date, processing_baseline, relative_orbit])


def get_recordings(query_result: list[dict[str, str]]) -> dict[str, dt.date]:
    
    recordings = [{'id' : res['id'], 'date': parse_date(res['properties']['datetime'])} for res in query_result]
...
def query_copernicushub(config: SHConfig, 
                        bbox: BBox, 
                        timeframe: tuple[dt.date]) -> list[dict[str, Union[str, dt.date]]]:
...
def download_single_satellite_image(evalscript: str, 
                             config: SHConfig, 
                             bbox: BBox, 
                             ...
                             resolution: int = 60,
                             max_cloud_coverage: float = 0.2) -> None:
    ...
    request = SentinelHubRequest(
        ...
        input_data=[
            SentinelHubRequest.input_data(
                ...
                time_interval=sorted(tuple(map(lambda x: x.strftime("%Y-%m-%d"), time_interval))),
...
def load_satellite_images(aoi: dict[str, Union[shapely.box, pyproj.crs.crs.CRS]],
                          time_range: tuple[dt.date],
                          file_path: Path,
                          show_progress: bool = False,
                          tmp_dir: Path = Path("tmp")
                         ) -> dict:
    config = create_configuration("sh-e670f9d0-df66-4f6f-aed0-fccc6f1f871f", "Iih2YwJFxVydRB7HG9j2x6PRn1M74GxP")
    ...
    for item in tqdm(available_recordings, disable=not show_progress):
        ...
        if not item['path'].is_file():
            download_single_satellite_image(evalscript,
                                            config,
                                            bbox, 
                                            time_range,
                                            item['path'])
